...
public class CustomBindAuthenticator extends AbstractLdapAuthenticator {
	...
	public DirContextOperations authenticate(Authentication authentication) {
		...
		String username = authentication.getName().replaceAll("(?i)@DISYS.COM", "");;
		String password = (String) authentication.getCredentials();

		if (!StringUtils.hasLength(password)) {
			logger.debug("Rejecting empty password for user " + username);
			throw new BadCredentialsException(messages.getMessage(
					"CustomBindAuthenticator.emptyPassword", "Empty Password"));
		}
		...
		if (user == null) {
			throw new BadCredentialsException(messages.getMessage(
					"CustomBindAuthenticator.badCredentials", "Bad credentials"));
		}
